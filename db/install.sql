...
--
--  این فایل ترکیب چهار فایل زیر به ترتیب درست است:
--    db/schema.sql                  ساخت ۱۳ جدول
--    db/seed/01-pages.sql           ۸۲ آدرس با متای سئو
--    db/seed/02-clinics-doctors.sql ۹ کلینیک و ۱۲ پزشک
--    db/seed/03-content.sql         متن ۷۷ صفحه
--
--  در phpMyAdmin فقط همین یک فایل را Import کنید.
--  اجرای مکرر بی‌خطر است: جدول‌ها با IF NOT EXISTS ساخته
--  می‌شوند و درج‌ها ON DUPLICATE KEY UPDATE دارند.
--
--  بازتولید: bash db/build-install.sh
-- ============================================================
...
<h2>سوالات متداول</h2>
<h3>آیا کاهش علائم همیشه به معنای موفقیت درمان است؟</h3>
<p>خیر. گاهی داروها می‌توانند برخی علائم را موقتاً کنترل کنند اما تأثیر قابل توجهی بر خود تومور نداشته باشند، و از طرف دیگر ممکن است بیمار در طول درمان با خستگی، تهوع یا سایر عوارض روبه‌رو باشد اما نتایج تصویربرداری نشان دهد که تومور در حال کوچک شدن است. به همین دلیل پزشکان معمولاً مجموعه‌ای از اطلاعات را کنار هم قرار می‌دهند تا درباره اثربخشی درمان تصمیم بگیرند.</p>
<h3>بعد از چند جلسه می‌توان نتیجه را فهمید؟</h3>
<p>زمان ارزیابی به نوع سرطان، داروهای مورد استفاده و برنامه درمانی بستگی دارد و در بسیاری از موارد پس از چند دوره شیمی‌درمانی نخستین تصویربرداری کنترل انجام می‌شود. بعضی بیماران در همان ماه‌های ابتدایی نشانه‌های پاسخ به درمان را نشان می‌دهند و در برخی دیگر ارزیابی نهایی به زمان بیشتری نیاز دارد.</p>
<h3>چه زمانی پزشک درمان را تغییر می‌دهد؟</h3>
<p>اگر بررسی‌ها نشان دهد که سرطان همچنان در حال رشد است یا پاسخ مناسبی به داروها نمی‌دهد، پزشک ممکن است نوع داروها را تغییر دهد یا از روش‌های دیگری مانند ایمونوتراپی، درمان هدفمند، جراحی یا پرتودرمانی استفاده کند و هدف این تصمیمات، انتخاب مؤثرترین روش برای کنترل بیماری است.</p>
<h3>از کجا بفهمیم شیمی درمانی جواب داده؟</h3>
<p>پاسخ شیمی‌درمانی معمولاً با بررسی علائم بیمار و نتایج آزمایش‌ها مشخص می‌شود که کاهش درد، بهبود اشتها، افزایش انرژی یا کمتر شدن علائم بیماری می‌تواند نشانه امیدوارکننده‌ای باشد اما معیار اصلی ارزیابی تصویربرداری‌هایی مانند سی‌تی‌اسکن، ام‌آر‌آی یا پت‌اسکن است و اگر تومور کوچک‌تر شود یا رشد آن متوقف شود و دیگر در تصاویر دیده نشود درمان مؤثر بوده است، در مقابل بزرگ‌تر شدن تومور یا ایجاد ضایعات جدید ممکن است نشان‌دهنده پاسخ ناکافی به شیمی‌درمانی باشد.</p>
...
<h2>سوالات متداول</h2>
<p>
<strong>آیا گرفتگی عروق پا قابل درمان است؟</strong>
</p>
...
<p>
<strong>آیا گرفتگی عروق پا با واریس تفاوت دارد؟</strong>
</p>
...
<p>
<strong>چقدر طول می‌کشد تا گرفتگی عروق پا پیشرفت کند؟</strong>
</p>
...
<p>
<strong>آیا ورزش برای گرفتگی عروق پا مفید است؟</strong>
</p>
...
<p>
<strong>آیا گرفتگی عروق پا می‌تواند منجر به سکته قلبی شود؟</strong>
</p>
...
<p>
<strong>آیا می‌توانم با گرفتگی عروق پا به زندگی عادی ادامه دهم؟</strong>
</p>
<p>بله، با تشخیص به موقع، درمان مناسب و رعایت توصیه‌های پزشکی، بسیاری از بیماران می‌توانند زندگی فعال و عادی داشته باشند. نکته کلیدی مدیریت بیماری و پیشگیری از پیشرفت آن است.</p>
<h2>جمع‌بندی نهایی</h2>
...
<h3>آیا تپش قلب همیشه نشانه آریتمی خطرناک است؟</h3>
<p>خیر، بسیاری از تپش‌ها خوش‌خیم هستند و ناشی از استرس، کافئین یا ورزش می‌باشند. اما اگر تداوم داشت یا با علائم دیگر همراه بود، باید بررسی شود.</p>
<h3>آیا می‌توانم با گوشی موبایل نوار قلب بگیرم؟</h3>
<p>برخی اپلیکیشن‌ها و گجت‌های متصل به گوشی این امکان را می‌دهند، اما دقت آن‌ها به اندازه دستگاه‌های پزشکی نیست و فقط برای غربالگری اولیه کاربرد دارند، نه تشخیص قطعی.</p>
<h3>درمان آریتمی چگونه است؟</h3>
<p>درمان بستگی به نوع آریتمی دارد و می‌تواند شامل دارو، تغییر سبک زندگی، شوک الکتریکی (کاردیوورژن)، ابلیشن (سوزاندن کانون آریتمی) یا نصب باتری قلب (Pacemaker) باشد.</p>
<p>متخصصین ما در تمامی موارد در کنار شما هستند. برای مشاوره تخصصی و راهنمایی، فرم زیر را تکمیل کنید تا در اسرع وقت با شما تماس بگیریم.</p>
<h2>فرم صفحه اصلی</h2>
...
<ol>
<li>
<b>چقدر طول می‌کشد تا سیستم ایمنی بعد از شیمی‌درمانی به حالت عادی برگردد؟</b>
<br>
این زمان برای هر فرد متفاوت است و بستگی به نوع داروهای مصرفی، سن و سلامت کلی شما دارد. اما به طور معمول، سیستم ایمنی بین چند هفته تا چند ماه (معمولا 1 تا 3 ماه) زمان نیاز دارد تا به‌طور کامل بازسازی شود.</li>
<li>
<b> آیا می‌توانم بعد از شیمی‌درمانی واکسن آنفولانزا بزنم؟</b>
<br>
بله، معمولا پزشکان توصیه می‌کنند، اما زمان‌بندی آن بسیار مهم است. چون سیستم ایمنی ضعیف است، ممکن است واکسن به خوبی عمل نکند. حتما قبل از تزریق هر واکسنی با پزشک انکولوژیست خود مشورت کنید. واکسن‌های حاوی ویروس زنده اغلب ممنوع هستند.</li>
<li>
<b>بهترین میان‌وعده برای تقویت بدن در این دوران چیست؟</b>
<br>
مخلوطی از مغزهای بوداده (بدون نمک زیاد و تمیز)، تکه‌های میوه پخته شده (مثل کمپوت سیب خانگی با کمترین شکر)، تخم‌مرغ آب‌‌پز سفت و اسموتی‌های خانگی با پودر پروتئین (تحت نظر پزشک) گزینه‌های عالی هستند.</li>
</ol>
<p>متخصصین ما در تمامی موارد در کنار شما هستند. برای مشاوره تخصصی و راهنمایی، فرم زیر را تکمیل کنید تا در اسرع وقت با شما تماس بگیریم.</p>
<h2>فرم صفحه اصلی</h2>
...
<ol>
<li>
<b>آیا باید شب قبل از شیمی‌درمانی ناشتا باشم؟</b>
<br>
خیر، مگر اینکه پزشک شما دستور خاصی داده باشد. ناشتا بودن طولانی باعث ضعف و افت قند خون می‌شود. بدن شما برای مقابله با استرس درمان به سوخت نیاز دارد. یک شام سبک و یک صبحانه مختصر معمولا توصیه می‌شود.</li>
<li>
<b>اگر اشتها ندارم، باز هم باید غذا بخورم؟</b>
<br>
بله، اما مجبور نیستید غذای جامد بخورید. اگر اشتها ندارید، سراغ مایعات مغذی بروید. اسموتی‌های خانگی (بدون لبنیات سنگین)، سوپ‌های رقیق یا آب‌میوه‌های طبیعی (مثل آب سیب) گزینه‌های خوبی هستند تا معده خالی نماند.</li>
<li>
<b>آیا می‌توانم مکمل یا ویتامین مصرف کنم؟</b>
<b>
<br>
</b>بسیار احتیاط کنید. برخی ویتامین‌ها و آنتی‌اکسیدان‌های قوی ممکن است با داروهای شیمی‌درمانی تداخل داشته باشند و اثر آن‌ها را کم کنند. حتماً لیست مکمل‌های خود را با پزشک آنکولوژیست خود چک کنید و خودسرانه شب قبل چیزی مصرف نکنید.</li>
<li>
<b>نوشیدن آب را تا کی ادامه دهم؟</b>
<br>
نوشیدن آب را در طول روز و شب ادامه دهید. حتی صبح روز درمان هم نوشیدن آب بسیار کمک‌کننده است. این کار رگ‌گیری را برای پرستار آسان‌تر کرده و کلیه‌ها را آماده پاکسازی می‌کند.</li>
<li>
<b>شب قبل از شیمی‌درمانی چه غذاهایی بخوریم؟</b>
<br>
برای شب قبل از شیمی‌درمانی بهتر است غذاهای سبک و سرشار از پروتئین مصرف کنید تا بدن برای درمان آماده‌تر باشد، همچنین بهتر است از غذاهای چرب، سرخ‌کردنی و پرادویه در شب قبل از شیمی‌درمانی پرهیز شود، برای شب قبل از شیمی درمانی مواد غذایی زیر پیشنهاد میشود:<br>
• تخم‌مرغ آب‌پز یا املت کم‌چرب<br>
• مرغ یا بوقلمون بدون پوست<br>
• ماهی به‌ویژه ماهی‌های چرب مانند سالمون<br>
• حبوبات مانند عدس و لوبیا<br>
• برنج، نان سبوس‌دار یا سیب‌زمینی<br>
• سبزیجات پخته و میوه‌های تازه<br>
• آب و مایعات کافی</li>
</ol>
</li>
</ol>
...
<ol>
<li>
<b>اگر یک جلسه را به دلیل بیماری یا مشکل شخصی از دست بدهم چه می‌شود؟</b>
<br>
اگر به‌خاطر سرماخوردگی، تب یا مشکلات شخصی یک جلسه به تعویق افتاد، پزشک معمولا برنامه را بازتنظیم می‌کند. اما باید سعی کنید تا حد امکان دقیق باشید. تاخیرهای مکرر می‌تواند اثر درمان را به شدت کاهش دهد. همیشه قبل از کنسل کردن نوبت، با تیم درمان مشورت کنید.</li>
<li>
<b>آیا شیمی درمانی همیشه 3 هفته یکبار است یا می‌تواند هر روز باشد؟</b>
<br>
همانطور که گفته شد، رایج‌ترین مدل هر 3 هفته یا 2 هفته یکبار است. اما برخی رژیم‌ها شامل مصرف قرص‌های خوراکی شیمی درمانی هستند که باید هر روز به مدت 14 روز در خانه مصرف شوند. همچنین در پرتودرمانی همزمان با شیمی درمانی، ممکن است دوزهای بسیار پایین شیمی درمانی به‌صورت هفتگی تزریق شود. پس مدل هر روز برای تزریق وریدی سنگین نادر است، اما برای قرص‌های خوراکی رایج است.</li>
<li>
<b>طولانی‌ترین دوره شیمی درمانی چقدر است؟</b>
<br>
برای شیمی درمانی نگهدارنده (Maintenance Therapy) که بعد از دوره اصلی انجام می‌شود یا برای کنترل سرطان‌های مزمن، درمان ممکن است سال‌ها طول بکشد. در این حالت، بیمار دوزهای پایین‌تری دریافت می‌کند تا بیماری خاموش بماند. اما برای شیمی درمانی‌های تهاجمی اولیه، معمولا دوره بیشتر از 6 تا 8 ماه توصیه نمی‌شود زیرا ریسک آسیب‌های دائمی به بدن بالا می‌رود.</li>
<li>
<b>شیمی‌درمانی چند جلسه است و هر جلسه با چه فاصله‌ای انجام می‌شود؟</b>
<br>
تعداد جلسات شیمی‌درمانی و فاصله بین جلسات به نوع سرطان، داروها و وضعیت بیمار بستگی دارد اما معمولاً در یک دوره درمان بین ۴ تا ۸ سیکل شیمی‌درمانی انجام می‌شود.<br>
• تعداد جلسات معمولاً ۴ تا ۸ سیکل است.<br>
• فاصله بین جلسات اغلب ۲ تا ۴ هفته می‌باشد.<br>
• هر جلسه ممکن است از چند دقیقه تا چند ساعت طول بکشد.<br>
• نوع درمان می‌تواند سرپایی یا بستری کوتاه‌مدت باشد.<br>
• برنامه درمانی توسط پزشک و بر اساس شرایط بیمار تنظیم می‌شود.</li>
</ol>
<p>متخصصین ما در تمامی موارد در کنار شما هستند. برای مشاوره تخصصی و راهنمایی، فرم زیر را تکمیل کنید تا در اسرع وقت با شما تماس بگیریم.</p>
<h2>فرم صفحه اصلی</h2>
...
<h2>سوالات متداول درباره بخش قلب و عروق</h2>
<p>‫۱. کاردیوآنکولوژی چیست و چرا برای بیماران تحت درمان سرطان ضروری است؟</p>
<p>‫کاردیوآنکولوژی شاخه‌ای تخصصی است که بر پیشگیری، تشخیص و مدیریت عوارض قلبی-عروقی ناشی از درمان‌های سرطان (مانند شیمی‌درمانی و رادیوتراپی) تمرکز دارد. بسیاری از داروهای ضدسرطان می‌توانند بر عملکرد قلب تأثیر بگذارند. حضور فلوشیپ کاردیوآنکولوژی در تیم درمان، تضمین می‌کند که پروسه درمان سرطان بدون آسیب به قلب پیش رود و سلامت قلب بیمار در تمام مراحل مراقبت شود.</p>
<p>‫۲. رویکرد «تیم چندتخصصی» (Multidisciplinary) در همراه کلینیک چه مزیتی برای بیمار دارد؟</p>
<p>‫برخلاف مراکز معمولی که بیمار را بین متخصصان مختلف سرگردان می‌کنند، در همراه کلینیک، فلوشیپ‌های قلب (اکوکاردیوگرافی، نارسایی قلب، کاردیوآنکولوژی) به همراه فوق تخصصان غدد، عفونی، روانپزشک و تغذیه، پرونده بیمار را به صورت مشترک بررسی می‌کنند. این هماهنگی به ویژه برای بیماران پیچیده (مانند دیابتی‌های دارای بیماری قلبی یا بیماران سرطانی) حیاتی است تا تداخلات دارویی کاهش یافته و بهترین استراتژی درمانی با کمترین عارضه جانبی انتخاب شود.</p>
<p>‫۳. تفاوت اکوکاردیوگرافی پیشرفته و استرس اکو در این مرکز با مراکز دیگر چیست؟</p>
<p>ما از دستگاه اکوکاردیوگرافی فیلیپس مدل افنیتی ادونس (Affinityَ Advance ) استفاده می‌کنیم که دقت تصویربرداری بسیار بالایی دارد. در اکوکاردیوگرافی پیشرفته بر خلاف برخی مراکز تمامی اندازه گیری های مورد نیاز در قلب و اطلاعاتی نظیر GLS(Global Longitudinal strain) ثبت میشود که در روند درمان بیماران، بخصوص بیماران سرطانی بسیار تعیین کننده میباشد. استرس اکوکاردیوگرافی(اکوکاردیوگرافی با ضربان بالا) تستی بی خطر، دقیق و حساس برای تشخیص زودهنگام تنگی‌های خاموش کرونر و مشکلات دریچه‌ای می باشد و برای تعیین ریسک قلبی قبل از هر عمل جراحی بسیار کمک کننده است. در این تست آنچه که دقت را بالا می برد تجربه پزشک انجام دهنده و کیفیت بالای دستگاه است که هر دو در همراه کلینیک فراهم شده است.</p>
<p>‫۴. آیا بیماران مبتلا به نارسایی قلبی پیشرفته نیز در این مرکز تحت پوشش هستند؟</p>
<p>‫بله، با حضور فلوشیپ تخصصی نارسایی قلب و واحد تزریقات (Infusion Unit)، ما امکان ارائه درمان‌های پیشرفته تزریقی برای بیماران نارسایی قلبی را فراهم کرده‌ایم. همچنین با همکاری متخصص تغذیه و روانپزشک، جنبه‌های سبک زندگی و سلامت روان این بیماران نیز به طور همزمان مدیریت می‌شود تا کیفیت زندگی آن‌ها به حداکثر برسد.</p>
<h2>جمع‌بندی نهایی</h2>
...
<h2>سوالات متداول درباره بخش شیمی‌درمانی</h2>
<p>۱. آیا شیمی‌درمانی دردناک است؟</p>
<p>خیر، خود فرآیند تزریق داروی شیمی‌درمانی معمولاً بدون درد است. ممکن است در محل ورود آنژیوکت احساس سوزش خفیفی داشته باشید که کاملاً قابل تحمل است. برخی داروها مانند پاکلی‌تاکسل ممکن است باعث گزگز دست و پا شوند که با داروهای پیش‌دارویی کنترل می‌شود.</p>
<p>۲. مدت زمان هر جلسه شیمی‌درمانی چقدر است؟</p>
<p>مدت زمان هر جلسه بسته به رژیم درمانی متفاوت است؛ از ۳۰ دقیقه تا ۸ ساعت متغیر است. برخی پروتکل‌ها به صورت تزریق سریع و برخی به صورت انفوزیون طولانی‌مدت انجام می‌شوند. در همراه کلینیک، فضای استراحت مناسبی برای بیماران در طول انفوزیون‌های طولانی در نظر گرفته شده است.</p>
<p>۳. آیا ریزش مو در همه بیماران رخ می‌دهد؟</p>
<p>ریزش مو به نوع داروی مصرفی بستگی دارد. داروهایی مانند دوکسوروبیسین، پاکلی‌تاکسل و سیکلوفسفامید معمولاً باعث ریزش مو می‌شوند، در حالی که بسیاری از داروهای هدفمند جدید این عارضه را ندارند. پس از پایان درمان، موها معمولاً طی ۳ تا ۶ ماه دوباره رشد می‌کنند.</p>
<p>۴. آیا در طول شیمی‌درمانی می‌توانم به سر کار برگردم؟</p>
<p>بسیاری از بیماران با تنظیم ساعات کاری و رعایت مراقبت‌ها، قادر به ادامه فعالیت حرفه‌ای خود هستند. این موضوع به نوع شغل، رژیم درمانی و وضعیت عمومی بیمار بستگی دارد. در همراه کلینیک، امکان تنظیم زمان جلسات در روزهای تعطیل یا ساعات شناور برای بیماران شاغل فراهم است.</p>
<h2>جمع‌بندی نهایی</h2>
...
<h2>سوالات متداول درباره بخش روان‌پزشکی کلینیک</h2>
<h4>۱. آیا برای مراجعه به بخش روان‌پزشکی نیاز به معرفی‌نامه از پزشک دیگر دارم؟</h4>
<p>خیر، در اکثر کلینیک‌ها می‌توانید مستقیماً و بدون نیاز به ارجاع، نوبت روان‌پزشکی بگیرید. البته در برخی سیستم‌های بیمه‌ای، داشتن معرفی‌نامه ممکن است به پوشش بهتر هزینه‌ها کمک کند.</p>
<h4>۲. جلسه اول روان‌پزشکی شامل چه مواردی می‌شود؟</h4>
<p>جلسه اول شامل مصاحبه بالینی جامع، بررسی سابقه خانوادگی، سابقه پزشکی، علائم فعلی و در صورت نیاز، انجام تست‌های روان‌سنجی است. در پایان جلسه، تشخیص اولیه و طرح درمان پیشنهادی ارائه می‌شود.</p>
<h4>۳. آیا داروهای روان‌پزشکی اعتیادآور هستند؟</h4>
<p>بیشتر داروهای رایج مانند ضدافسردگی‌ها (SSRIها) و ضدروان‌پریشی‌ها اعتیادآور نیستند. تنها برخی از ضد اضطراب‌ها مانند بنزودیازپین‌ها در صورت مصرف طولانی‌مدت ممکن است وابستگی ایجاد کنند که روان‌پزشک این موضوع را مدیریت می‌کند.</p>
<h4>۴. طول دوره درمان روان‌پزشکی معمولاً چقدر است؟</h4>
<p>بسته به نوع اختلال متفاوت است. افسردگی خفیف ممکن است ۶ تا ۱۲ ماه درمان نیاز داشته باشد، در حالی که اختلالاتی مانند دوقطبی یا اسکیزوفرنی نیازمند درمان مادام‌العمر هستند. روان‌پزشک بر اساس پاسخ درمانی، مدت را تنظیم می‌کند.</p>
<h4>۵. آیا اطلاعات من در بخش روان‌پزشکی محرمانه می‌ماند؟</h4>
<p>بله، رازداری یکی از اصول بنیادین اخلاق روان‌پزشکی است. اطلاعات شما تنها در موارد استثنایی مانند خطر جدی برای جان خود یا دیگران، یا به حکم قانون، قابل افشا است. در سایر موارد، هیچ اطلاعاتی بدون رضایت کتبی شما به شخص ثالثی داده نمی‌شود.</p>
<h2>جمع‌بندی نهایی</h2>
...
<h2>سوالات متداول درباره خدمات جدید</h2>
<h4>۱. تفاوت روان‌درمانی MCT با روش‌های معمول چیست؟</h4>
<p>در فراشناخت‌درمانی (MCT)، به جای بحث درباره محتوای افکار منفی، بر روی «نحوه فکر کردن» و کنترل توجه تمرکز می‌شود. این روش برای افرادی که دچار نشخوار فکری شدید یا نگرانی مداوم هستند، بسیار کارآمدتر و سریع‌تر از روش‌های سنتی عمل می‌کند.</p>
<h4>۲. آیا خدمات سازمانی فقط برای بیمارستان‌هاست؟</h4>
<p>خیر. اگرچه تخصص ما در بهبود تجربه مراجعان مراکز درمانی است، اما اصول روانشناسی صنعتی و سازمانی ما برای تمام شرکت‌ها، استارتاپ‌ها و نهادهای دولتی جهت ارزیابی نیروی انسانی، کاهش استرس شغلی و بهبود رضایت مشتریان قابل اجراست.</p>
<h4>۳. فرآیند ارزیابی روان‌شناختی چگونه است؟</h4>
<p>پس از جلسه مصاحبه اولیه، بسته به نیاز شما، مجموعه‌ای از آزمون‌های استاندارد (کاغذی یا کامپیوتری) تکمیل می‌شود. سپس نتایج توسط متخصص تفسیر شده و یک گزارش جامع همراه با پیشنهادات درمانی یا توسعه‌ای به شما ارائه می‌گردد.</p>
<h3>دریافت مشاوره تخصصی و ارزیابی</h3>
<p>برای رزرو وقت ارزیابی یا دریافت اطلاعات درباره قراردادهای سازمانی تماس بگیرید.</p>
<p>
<a>📞 ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/روان-شناسی/';
...
<h2>سوالات متداول درباره بخش تغذیه کلینیک</h2>
<h4>۱. آیا برای مراجعه به بخش تغذیه نیاز به معرفی‌نامه پزشک دارم؟</h4>
<p>خیر، در اکثر کلینیک‌ها می‌توانید مستقیماً و بدون نیاز به ارجاع، نوبت متخصص تغذیه بگیرید. با این حال، اگر بیماری زمینه‌ای مانند دیابت، کم‌خونی یا اختلال تیروئید دارید، همراه‌داشتن آزمایش‌های اخیر و معرفی‌نامه پزشک معالج، به طراحی دقیق‌تر رژیم کمک می‌کند.</p>
<h4>۲. جلسه اول متخصص تغذیه شامل چه مواردی می‌شود؟</h4>
<p>جلسه اول معمولاً ۶۰ تا ۹۰ دقیقه طول می‌کشد و شامل بررسی سابقه پزشکی و خانوادگی، ارزیابی عادات غذایی، بررسی فعالیت بدنی و در صورت نیاز، بررسی آزمایش‌های خون است. در پایان جلسه، نیاز کالری و درشت‌مغذی‌های شما محاسبه شده و رژیم اولیه ارائه می‌شود.</p>
<h4>۳. آیا رژیم‌های کلینیک شامل حذف کامل نان و برنج هستند؟</h4>
<p>خیر، رویکرد علمی و اصولی در تغذیه بالینی، حذف کامل گروه‌های غذایی نیست مگر در موارد خاص پزشکی (مانند سلیاک برای گلوتن). متخصص تغذیه مجرب، با محاسبه نیاز کالری و کربوهیدرات شما، مقدار مناسب نان، برنج و سایر کربوهیدرات‌ها را در رژیم می‌گنجاند.</p>
<h4>۴. مدت زمان لازم برای رسیدن به وزن ایده‌آل چقدر است؟</h4>
<p>کاهش وزن سالم و پایدار، معمولاً ۰.۵ تا ۱ کیلوگرم در هفته است. برای کاهش ۱۰ کیلوگرم اضافه وزن، به حدود ۳ تا ۶ ماه زمان نیاز دارید. پس از رسیدن به هدف، یک دوره تثبیت ۳ تا ۶ ماهه برای تنظیم متابولیسم و جلوگیری از بازگشت وزن ضروری است.</p>
<h4>۵. آیا متخصص تغذیه مکمل هم تجویز می‌کند؟</h4>
<p>بله، در صورت تشخیص کمبود ریزمغذی از طریق آزمایش خون یا ارزیابی رژیم غذایی، متخصص تغذیه می‌تواند مکمل‌های مناسب مانند ویتامین D، آهن، B12، امگا ۳ یا پروبیوتیک را تجویز کند. با این حال، اصل اول در تغذیه بالینی، دریافت ریزمغذی‌ها از طریق غذا است.</p>
<h2>جمع‌بندی نهایی</h2>
...
<h2>سوالات متداول والدین درباره قلب کودکان</h2>
<h4>۱. آیا هر صدای اضافی قلب (سوفل) در کودکان خطرناک است؟</h4>
<p>خیر، بسیاری از سوفل‌ها در کودکان &#8220;بی‌خطر&#8221; (Innocent) هستند و با رشد کودک برطرف می‌شوند. با این حال، تشخیص قطعی بی‌خطر بودن آن تنها توسط فلوشیپ قلب کودکان و با انجام اکوکاردیوگرافی امکان‌پذیر است.</p>
<h4>۲. اکوکاردیوگرافی جنینی چیست و چه کسانی باید انجام دهند؟</h4>
<p>یک سونوگرافی تخصصی و دقیق از قلب جنین است که معمولاً بین هفته‌های ۱۸ تا ۲۲ بارداری برای مادران پرخطر (مانند سابقه خانوادگی بیماری قلبی، دیابت بارداری، یا مصرف داروهای خاص) توصیه می‌شود.</p>
<h4>۳. آیا کودک مبتلا به بیماری قلبی می‌تواند ورزش کند یا بازی کند؟</h4>
<p>بسته به نوع و شدت بیماری، بسیاری از کودکان می‌توانند فعالیت‌های سبک یا حتی ورزش‌های رقابتی را با تایید پزشک انجام دهند. محدود کردن خودسرانه و بدون دلیل فعالیت کودک، توصیه نمی‌شود.</p>
<h4>۴. تفاوت ویزیت قلب کودکان با قلب بزرگسالان چیست؟</h4>
<p>بیماری‌های قلبی کودکان اغلب مادرزادی (Congenital) هستند و فیزیولوژی قلب در حال رشد، تفسیر علائم و دوز دارویی کاملاً متفاوتی دارد که فقط فلوشیپ فوق تخصصی قلب کودکان آموزش دیده این دانش را دارد.</p>
<h4>۵. آیا در صورت نیاز به جراحی، کلینیک هماهنگی لازم را انجام می‌دهد؟</h4>
<p>بله، یکی از مزایای مراجعه به یک کلینیک جامع، داشتن شبکه ارتباطی قوی با برترین جراحان قلب کودکان است. در صورت نیاز، پرونده بیمار با هماهنگی کامل و معرفی‌نامه معتبر ارجاع داده می‌شود.</p>
<h2>جمع‌بندی نهایی</h2>
...
<h2>سؤالات متداول</h2>
<strong>آیا خستگی مزمن من می‌تواند از آدرنال باشد؟</strong>خستگی مزمن علل بسیاری دارد. نارسایی واقعی آدرنال یک بیماری مشخص و قابل‌تشخیص با آزمایش است. ارزیابی پزشک غدد می‌تواند آن را تأیید یا رد کند.
<strong>فشار خون مقاوم به درمان چه ارتباطی با آدرنال دارد؟</strong>گاهی فشار خونی که با چند دارو کنترل نمی‌شود، ریشه‌ی هورمونی دارد (مانند هیپرآلدوسترونیسم). در این موارد ارزیابی آدرنال توصیه می‌شود.
<h3>ارزیابی تخصصی غده فوق کلیه</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/pituitary/">اختلالات هیپوفیز</a> · <a href="/service/endocrine-clinic/metabolic-syndrome/">سندرم متابولیک</a>
...
<h2>سؤالات متداول</h2>
<strong>آیا زخم شریانی قابل درمان است؟</strong>
<p>بله، با تشخیص به‌موقع و revascularization، اکثر زخم‌های شریانی قابل درمان هستند. تأخیر منجر به قطع عضو می‌شود.</p>
<strong>چرا درد شب‌ها بدتر می‌شود؟</strong>
<p>در حالت دراز کشیدن، جاذبه‌ی زمین در رساندن خون به پاها کمک نمی‌کند. این یک علامت کلاسیک ایسکمی پیشرفته است.</p>
<strong>HBOT چقدر مؤثر است؟</strong>
<p>HBOT می‌تواند در زخم‌های شریانی پس از revascularization، روند بهبود را تسریع کند. اما به‌تنهایی جایگزین درمان عروقی نیست.</p>
<h3>ارزیابی فوری زخم شریانی</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>
<h3>سایر زخم‌ها</h3>
<p>
<a href="/service/venous-ulcer/">زخم وریدی</a> · <a href="/service/diabetic-foot/">زخم پای دیابتی</a>
</p>' WHERE path = '/service/arterial-ulcer/';
...
<h2>سؤالات متداول</h2>
<strong>آیا سوختگی درجه ۲ خود به خود خوب می‌شود؟</strong>
<p>سوختگی درجه ۲ سطحی معمولاً در ۲-۳ هفته بدون اسکار بهبود می‌یابد. سوختگی درجه ۲ عمقی ممکن است ۳-۸ هفته نیاز داشته باشد و اسکار باقی بگذارد.</p>
<strong>آیا پماد سوختگی خانگی مؤثر است؟</strong>
<p>مالیدن خمیردندان، روغن، تخم‌مرغ و موارد سنتی روی سوختگی <strong>اکیداً ممنوع</strong> است و می‌تواند عفونت ایجاد کند. فقط آب سرد در ۲۰ دقیقه‌ی اول و سپس مراجعه به پزشک.</p>
<h3>ویزیت سرپایی سوختگی</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/burn-wound/';
...
<h2>سؤالات متداول</h2>
<strong>چرا زخم من ۶ ماه است که خوب نمی‌شود؟</strong>
<p>زخم‌های مزمن یک یا چند علت زمینه‌ای دارند که اگر شناسایی و درمان نشوند، زخم بهبود نمی‌یابد. ارزیابی جامع اولین قدم است.</p>
<strong>آیا قطع عضو حتمی است؟</strong>
<p>خیر. اکثر زخم‌های مزمن با درمان مناسب و در زمان مناسب بهبود می‌یابند. قطع عضو فقط در موارد گانگرن یا عفونت کنترل‌نشده‌ی تهدیدکننده‌ی جان لازم می‌شود.</p>
<strong>هزینه‌ی درمان زخم مزمن چقدر است؟</strong>
<p>به علت و وسعت زخم بستگی دارد. در اولین ویزیت برآورد دقیق ارائه می‌شود. این هزینه باید با هزینه‌ی عوارض زخم مزمن (بستری، جراحی، قطع عضو) مقایسه شود.</p>
<h3>ارزیابی جامع زخم مزمن</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>
<h3>انواع زخم‌های مزمن مرتبط</h3>
<p>
<a href="/service/diabetic-foot/">زخم پای دیابتی</a> · <a href="/service/venous-ulcer/">زخم وریدی</a> · <a href="/service/arterial-ulcer/">زخم شریانی</a> · <a href="/service/pressure-ulcer/">زخم بستر</a>
</p>' WHERE path = '/service/chronic-wound/';
...
<h2>سؤالات متداول</h2>
<strong>آیا دیابت نوع ۲ قابل برگشت است؟</strong>در مراحل اولیه و با کاهش وزن چشمگیر و تغییر سبک زندگی، بسیاری از بیماران می‌توانند قند خود را بدون دارو در محدوده‌ی نرمال نگه دارند (Remission). اما این موضوع نیاز به پیگیری مداوم دارد.
<strong>HbA1c چیست و چرا مهم است؟</strong>HbA1c میانگین قند خون شما در ۲ تا ۳ ماه گذشته را نشان می‌دهد. هدف برای اکثر بیماران دیابتی، زیر ۷٪ است. این آزمایش بهترین شاخص کنترل بلندمدت دیابت محسوب می‌شود.
<strong>آیا با دیابت می‌توانم میوه بخورم؟</strong>بله، اما با اندازه و انتخاب درست. میوه‌های با شاخص قند پایین و در حد متعادل توصیه می‌شوند. برنامه‌ی دقیق توسط کارشناس تغذیه‌ی کلینیک تنظیم می‌شود.
<h3>کنترل تخصصی دیابت</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/obesity/">چاقی و کنترل وزن</a> · <a href="/service/endocrine-clinic/metabolic-syndrome/">چربی خون و سندرم متابولیک</a> · <a href="/service/wound-clinic/diabetic-foot/">زخم پای دیابتی</a>
...
<p>زمان بهبود بستگی به عمق زخم، کنترل قند، وضعیت عروقی و تبعیت بیمار دارد. زخم‌های درجه ۱ ممکن است ۴-۶ هفته، اما زخم‌های عمیق‌تر یا با عفونت ممکن است ۳ تا ۶ ماه نیاز داشته باشند. در طول این مدت، ویزیت‌های منظم هر ۱-۲ هفته ضروری است.</p>
<h2>سؤالات متداول</h2>
<strong>آیا با زخم پای دیابتی می‌توان راه رفت؟</strong>
<p>تا حد امکان از فشار روی پای دارای زخم اجتناب کنید. پزشک ممکن است کفش طبی، کاست تخصصی یا عصای زیربغل تجویز کند.</p>
<strong>آیا قطع عضو حتمی است؟</strong>
<p>خیر. با تشخیص و درمان به‌موقع، اکثر زخم‌های پای دیابتی بدون نیاز به قطع عضو بهبود می‌یابند. تأخیر در درمان است که ریسک قطع را افزایش می‌دهد.</p>
<strong>NPWT برای زخم دیابتی چقدر مؤثر است؟</strong>
<p>مطالعات نشان داده NPWT می‌تواند زمان بهبود زخم‌های دیابتی را تا ۴۰٪ کاهش دهد، به‌خصوص در زخم‌های درجه ۲ و ۳.</p>
<strong>آیا بیمه هزینه‌ی درمان را پوشش می‌دهد؟</strong>
<p>بخشی از هزینه‌های ویزیت و پانسمان معمولاً تحت پوشش بیمه‌ی پایه است. هزینه‌ی NPWT و HBOT با بیمه‌ی تکمیلی ممکن است پوشش داده شود.</p>
<h3>برای رزرو نوبت زخم پای دیابتی</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>
<h3>سایر زخم‌هایی که درمان می‌کنیم</h3>
<p>
<a href="/service/pressure-ulcer/">زخم بستر</a> · <a href="/service/venous-ulcer/">زخم وریدی</a> · <a href="/service/arterial-ulcer/">زخم شریانی</a> · <a href="/service/chronic-wound/">زخم مزمن</a> · <a href="/service/infected-wound/">زخم عفونی</a>
</p>' WHERE path = '/service/diabetic-foot/';
...
<p>✓<br></p>
<p>مدیریت استرس و خواب کافی (کورتیزول بالا دشمن تعادل هورمونی است)</p>
<h2>سوالات متداول بیماران غدد</h2>
<h4>۱. آیا بیماری‌های غدد مثل تیروئید قابل درمان قطعی هستند؟</h4>
<p>بسیاری از اختلالات مانند کم‌کاری تیروئید با مصرف روزانه یک قرص ساده کاملاً کنترل می‌شوند و فرد زندگی عادی خواهد داشت. برخی موارد مانند گره‌های خوش‌خیم فقط نیاز به پیگیری دارند. هدف ما بازگرداندن تعادل هورمونی به بدن است.</p>
<h4>۲. ارتباط دیابت و زخم پای دیابتی چیست؟</h4>
<p>قند خون بالا به مرور زمان به اعصاب و عروق پا آسیب می‌زند. این باعث می‌شود بیمار متوجه زخم نشود و زخم بهبود نیابد. در کلینیک همراه، کنترل قند در بخش غدد و مراقبت از پا در بخش زخم به صورت هماهنگ انجام می‌شود.</p>
<h4>۳. هر چند وقت یکبار باید برای چکاپ غدد مراجعه کنم؟</h4>
<p>برای بیماری‌های پایدار مثل کم‌کاری تیروئید، معمولاً هر ۶ تا ۱۲ ماه کافی است. برای دیابت، پایش هر ۳ ماهه (HbA1c) ضروری است. پزشک شما برنامه دقیق پیگیری را تعیین خواهد کرد.</p>
<h3>تعادل هورمونی، کلید سلامتی</h3>
<p>برای دریافت نوبت ویزیت با فوق تخصص غدد و بررسی دقیق وضعیت متابولیک، همین حالا اقدام کنید.</p>
<p>
<a>📞 ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/endocrine-clinic/';
...
<h2>سؤالات متداول</h2>
<strong>کوتاهی قد فرزندم همیشه نیاز به درمان دارد؟</strong>خیر. بسیاری از کودکان کوتاه‌قد فقط الگوی رشد خانوادگی یا تأخیری دارند و کاملاً طبیعی‌اند. ارزیابی پزشک مشخص می‌کند که آیا علت قابل‌درمانی وجود دارد یا خیر.
<strong>بهترین زمان مراجعه برای درمان رشد چه موقع است؟</strong>هرچه زودتر بهتر. درمان اختلالات رشد پیش از بسته شدن صفحات رشد استخوان مؤثرتر است، بنابراین مراجعه‌ی به‌موقع اهمیت زیادی دارد.
<h3>ارزیابی رشد و بلوغ کودک</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/thyroid/">بیماری‌های تیروئید</a> · <a href="/service/endocrine-clinic/pituitary/">اختلالات هیپوفیز</a>
...
<h2>سؤالات متداول</h2>
<strong>چه زمانی آنتی‌بیوتیک خوراکی لازم است؟</strong>
<p>زمانی که عفونت از زخم به بافت اطراف پخش شده باشد (سلولیت) یا علائم سیستمیک (تب) وجود داشته باشد.</p>
<strong>آیا عسل واقعاً برای زخم مفید است؟</strong>
<p>عسل پزشکی استاندارد (Medical-grade Manuka Honey) دارای اثرات آنتی‌میکروبیال ثابت‌شده است. عسل معمولی خانگی توصیه نمی‌شود.</p>
<h3>ارزیابی فوری زخم عفونی</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/infected-wound/';
...
<p>✓<br></p>
<p>مراجعه سریع به پزشک در صورت مشاهده علائم عفونت زخم یا تب ناگهانی</p>
<h2>سوالات متداول بیماران عفونی</h2>
<h4>۱. آیا برای ویزیت نیاز به ارجاع از پزشک دیگری دارم؟</h4>
<p>خیر، شما می‌توانید مستقیماً از متخصص عفونی نوبت بگیرید. اما اگر آزمایش یا تصویربرداری خاصی دارید، همراه داشتن آن‌ها به تشخیص سریع‌تر کمک می‌کند.</p>
<h4>۲. آیا تست‌های HIV و هپاتیت محرمانه است؟</h4>
<p>بله، کاملاً. تمام مراحل مشاوره، نمونه‌گیری و اعلام نتایج با رعایت اصول اخلاقی و محرمانگی کامل انجام می‌شود و اطلاعات فقط در اختیار خود بیمار قرار می‌گیرد.</p>
<h4>۳. درمان سل (TB) چقدر طول می‌کشد؟</h4>
<p>درمان استاندارد سل حداقل ۶ ماه طول می‌کشد. مصرف منظم داروها در این دوره حیاتی است تا از بازگشت بیماری و مقاوم شدن میکرب جلوگیری شود.</p>
<h4>۴. تفاوت ویروس و باکتری چیست؟</h4>
<p>باکتری‌ها موجودات زنده‌ای هستند که با آنتی‌بیوتیک کشته می‌شوند، اما ویروس‌ها (مثل سرماخوردگی یا آنفلوآنزا) به آنتی‌بیوتیک پاسخ نمی‌دهند و درمان آن‌ها حمایتی یا ضدویروسی خاص است.</p>
<h3>تشخیص دقیق، درمان ایمن</h3>
<p>برای دریافت نوبت ویزیت با متخصص عفونی و انجام مشاوره تخصصی، همین حالا اقدام کنید.</p>
<p>
<a>📞 ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/infectious-clinic/';
...
<h2>سؤالات متداول</h2>
<strong>آیا چربی خون فقط با رژیم درست می‌شود؟</strong>در موارد خفیف، تغذیه و ورزش می‌تواند کافی باشد. اما اگر ریسک قلبی بالا یا چربی خیلی بالا باشد، دارو (مانند استاتین) برای پیشگیری از سکته ضروری است.
<strong>آیا مصرف استاتین مادام‌العمر است؟</strong>در بسیاری موارد بله، چون این داروها ریسک قلبی-عروقی را کاهش می‌دهند. تصمیم بر اساس ریسک فردی شما توسط پزشک گرفته می‌شود.
<h3>ارزیابی جامع متابولیک</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/diabetes/">دیابت</a> · <a href="/service/endocrine-clinic/obesity/">چاقی و کنترل وزن</a>
...
<h2>سؤالات متداول</h2>
<strong>چرا با وجود رژیم، وزنم کم نمی‌شود؟</strong>گاهی علل هورمونی مانند کم‌کاری تیروئید، مقاومت به انسولین یا PCOS مانع کاهش وزن می‌شوند. ارزیابی غدد می‌تواند این عوامل پنهان را شناسایی کند.
<strong>داروهای لاغری جدید (GLP-1) چقدر مؤثرند؟</strong>این داروها می‌توانند کاهش وزن قابل‌توجهی ایجاد کنند، اما باید تحت نظر پزشک، با اندیکاسیون درست و در کنار تغذیه و ورزش تجویز شوند. خوددرمانی توصیه نمی‌شود.
<h3>برنامه‌ی علمی کاهش وزن</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/metabolic-syndrome/">سندرم متابولیک</a> · <a href="/service/endocrine-clinic/diabetes/">دیابت</a> · <a href="/service/endocrine-clinic/pcos/">تخمدان پلی‌کیستیک</a>
...
<h2>سؤالات متداول</h2>
<strong>از چه سنی باید تست تراکم استخوان بدهم؟</strong>برای زنان معمولاً از زمان یائسگی یا ۶۵ سالگی، و زودتر در صورت وجود عوامل خطر (مصرف کورتون، شکستگی قبلی، لاغری شدید). پزشک بر اساس شرایط شما تصمیم می‌گیرد.
<strong>آیا فقط مصرف کلسیم برای درمان کافی است؟</strong>خیر. کلسیم و ویتامین D پایه‌اند، اما در پوکی استخوان واقعی معمولاً به داروهای اختصاصی برای کاهش خطر شکستگی نیاز است.
<h3>ارزیابی سلامت استخوان</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/sex-hormones/">هورمون‌های جنسی و یائسگی</a> · <a href="/service/endocrine-clinic/thyroid/">بیماری‌های تیروئید</a>
...
<h2>سؤالات متداول</h2>
<strong>آیا با PCOS می‌توانم باردار شوم؟</strong>بله. بسیاری از زنان مبتلا به PCOS با کاهش وزن، تنظیم هورمونی و در صورت نیاز القای تخمک‌گذاری باردار می‌شوند. درمان به‌موقع شانس باروری را افزایش می‌دهد.
<strong>آیا PCOS درمان قطعی دارد؟</strong>PCOS یک وضعیت مزمن است، اما کاملاً قابل کنترل است. با مدیریت سبک زندگی و درمان دارویی، علائم به‌خوبی کنترل و از عوارض بلندمدت پیشگیری می‌شود.
<strong>چرا PCOS با دیابت ارتباط دارد؟</strong>مقاومت به انسولین که در PCOS شایع است، خطر ابتلا به دیابت نوع ۲ را افزایش می‌دهد. به همین دلیل کنترل وزن و پایش قند در این بیماران اهمیت دارد.
<h3>مشاوره‌ی تخصصی PCOS</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/obesity/">چاقی و کنترل وزن</a> · <a href="/service/endocrine-clinic/sex-hormones/">هورمون‌های جنسی</a> · <a href="/service/endocrine-clinic/diabetes/">دیابت</a>
...
<h2>سؤالات متداول</h2>
<strong>آیا تومور هیپوفیز سرطانی است؟</strong>اکثریت قریب به اتفاق تومورهای هیپوفیز خوش‌خیم (آدنوم) هستند و بسیاری از آن‌ها فقط با دارو کنترل می‌شوند.
<strong>تفاوت دیابت بی‌مزه با دیابت قند چیست؟</strong>دیابت بی‌مزه به قند خون ربطی ندارد؛ مشکل در هورمون تنظیم آب بدن است که باعث تشنگی و ادرار فراوان می‌شود. درمان آن کاملاً متفاوت است.
<h3>ارزیابی تخصصی هیپوفیز</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/adrenal/">اختلالات غده فوق کلیه</a> · <a href="/service/endocrine-clinic/thyroid/">بیماری‌های تیروئید</a> · <a href="/service/endocrine-clinic/sex-hormones/">هورمون‌های جنسی</a>
...
<h2>سؤالات متداول</h2>
<strong>آیا زخم بستر در منزل قابل درمان است؟</strong>
<p>بله. در موارد سبک تا متوسط، تیم پرستاری و پزشکی ما به منزل اعزام می‌شود. در موارد شدید، انتقال به کلینیک ضروری است.</p>
<strong>چقدر طول می‌کشد زخم بستر خوب شود؟</strong>
<p>زخم مرحله ۱ ممکن است در ۳-۷ روز، مرحله ۲ در ۲-۴ هفته، اما مرحله ۳ و ۴ ممکن است ماه‌ها زمان نیاز داشته باشند.</p>
<strong>تشک ضد زخم بستر را از کجا تهیه کنیم؟</strong>
<p>کلینیک همراه با چند تأمین‌کننده‌ی معتبر همکاری دارد و می‌توانیم بهترین گزینه را با توجه به شرایط بیمار معرفی کنیم.</p>
<h3>برای درخواست ویزیت در منزل</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>
<h3>سایر زخم‌ها</h3>
<p>
<a href="/service/diabetic-foot/">زخم پای دیابتی</a> · <a href="/service/chronic-wound/">زخم مزمن</a> · <a href="/service/infected-wound/">زخم عفونی</a>
</p>' WHERE path = '/service/pressure-ulcer/';
...
<h2>سؤالات متداول</h2>
<strong>آیا هورمون‌درمانی یائسگی خطرناک است؟</strong>هورمون‌درمانی در افراد مناسب و با دوز و مدت درست، بی‌خطر و مؤثر است. تصمیم بر اساس سن، علائم و سابقه‌ی پزشکی فردی و پس از ارزیابی دقیق گرفته می‌شود.
<strong>افت تستوسترون فقط مربوط به سن است؟</strong>خیر. علاوه بر سن، چاقی، دیابت، اختلالات هیپوفیز و برخی داروها نیز می‌توانند باعث افت تستوسترون شوند. به همین دلیل ارزیابی علت ضروری است.
<h3>ارزیابی و تعادل هورمونی</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/osteoporosis/">پوکی استخوان</a> · <a href="/service/endocrine-clinic/pcos/">تخمدان پلی‌کیستیک</a> · <a href="/service/endocrine-clinic/pituitary/">اختلالات هیپوفیز</a>
...
<h2>سؤالات متداول</h2>
<strong>چه زمانی نگران زخم جراحی شویم؟</strong>
<p>قرمزی فزاینده، ترشح چرکی، تب، باز شدن لبه‌های زخم، درد فزاینده — همگی نیاز به ارزیابی فوری دارند.</p>
<strong>آیا NPWT برای همه‌ی زخم‌های جراحی مناسب است؟</strong>
<p>خیر. NPWT در زخم‌های با ترشح زیاد، عمق متوسط تا زیاد، و در زخم‌های Open Abdomen کاربرد دارد. در زخم‌های با خونریزی فعال یا عفونت شدید کنترل‌نشده، contraindicated است.</p>
<strong>هزینه‌ی NPWT چقدر است؟</strong>
<p>بسته به مدت و نوع دستگاه متفاوت است. در ویزیت اول برآورد دقیق ارائه می‌شود.</p>
<h3>ارزیابی زخم بعد از جراحی</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/surgical-wound/';
...
<h2>سؤالات متداول</h2>
<strong>آیا داروی تیروئید را باید مادام‌العمر مصرف کنم؟</strong>در کم‌کاری دائمی تیروئید (مانند هاشیموتو)، معمولاً مصرف لووتیروکسین مادام‌العمر است، اما دارو ارزان، بی‌خطر و کاملاً مؤثر است و زندگی طبیعی را ممکن می‌سازد.
<strong>آیا همه‌ی گره‌های تیروئید سرطانی هستند؟</strong>خیر. بیش از ۹۰٪ گره‌های تیروئید خوش‌خیم‌اند. با سونوگرافی و در صورت نیاز نمونه‌برداری، گره‌های پرخطر شناسایی می‌شوند.
<strong>قرص تیروئید را چه زمانی بخورم؟</strong>لووتیروکسین باید ناشتا و حدود ۳۰ تا ۶۰ دقیقه پیش از صبحانه با آب مصرف شود تا جذب کامل صورت گیرد.
<h3>سونوگرافی و ارزیابی تیروئید</h3>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
<h3>خدمات مرتبط</h3>
<a href="/service/endocrine-clinic/obesity/">چاقی و کنترل وزن</a> · <a href="/service/endocrine-clinic/diabetes/">دیابت</a>
...
<h2>سؤالات متداول</h2>
<strong>تا چه مدت پس از حادثه می‌توان بخیه زد؟</strong>
<p>بخیه‌ی اولیه معمولاً در ۶-۸ ساعت اول. در صورت تأخیر، خطر عفونت افزایش می‌یابد و ممکن است بخیه‌ی تأخیری توصیه شود.</p>
<strong>زخم گاز سگ چه باید کرد؟</strong>
<p>شست‌وشوی فوری با آب و صابون به مدت ۵ دقیقه، مراجعه به اورژانس برای واکسن هاری و کزاز، و سپس پیگیری در کلینیک زخم.</p>
<h3>پیگیری زخم ضربه‌ای</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/traumatic-wound/';
...
<h2>سؤالات متداول</h2>
<strong>آیا UTI بدون آنتی‌بیوتیک خوب می‌شود؟</strong>
<p>UTI ساده در برخی موارد خفیف می‌تواند خود به خود بهبود یابد، اما اکثر موارد نیاز به آنتی‌بیوتیک دارند تا از پیلونفریت جلوگیری شود.</p>
<strong>چرا UTI من مرتب عود می‌کند؟</strong>
<p>دلایل: تخلیه‌ی ناقص مثانه، سنگ ادراری، یائسگی، فعالیت جنسی، نقص ایمنی، یا مقاومت میکروبی. ارزیابی جامع لازم است.</p>
<h3>نوبت ویزیت عفونت ادراری</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/urinary-tract-infection/';
...
<p>✓<br></p>
<p>برخورد محترمانه و ایجاد حس آرامش برای بیمارانی که مشکلات حساس دارند</p>
<h2>سوالات متداول بیماران اورولوژی</h2>
<h4>۱. آیا خانم‌ها هم باید به متخصص اورولوژی مراجعه کنند؟</h4>
<p>بله، قطعاً. اورولوژی تخصص بیماری‌های دستگاه ادراری (کلیه، مثانه و مجرا) است که در هر دو جنس مشترک است. عفونت‌های مکرر ادراری، سنگ کلیه، بی‌اختیاری ادرار و خون در ادرار از مشکلات شایع بانوان است که توسط اورولوژیست درمان می‌شود.</p>
<h4>۲. برای ویزیت اورولوژی چه آمادگی‌هایی لازم است؟</h4>
<p>بهتر است مثانه شما پر باشد تا در صورت نیاز سونوگرافی انجام شود. همچنین اگر آزمایش ادرار یا خون اخیر دارید، همراه داشته باشید. برای معاینه پروستات در آقایان، تخلیه روده می‌تواند کمک‌کننده باشد اما الزامی نیست.</p>
<h4>۳. آیا سنگ کلیه همیشه نیاز به جراحی دارد؟</h4>
<p>خیر. سنگ‌های کوچک (زیر ۵-۶ میلی‌متر) معمولاً با مصرف مایعات و دارو دفع می‌شوند. سنگ‌های متوسط با سنگ‌شکنی سرپایی و تنها سنگ‌های بسیار بزرگ یا پیچیده نیاز به جراحی دارند.</p>
<h4>۴. درمان کرایوتراپی برای HPV چقدر طول می‌کشد؟</h4>
<p>این روش معمولاً به صورت سرپایی و در چند جلسه کوتاه انجام می‌شود. پس از فریز کردن ضایعه، ناحیه بهبود یافته و زگیل طی چند روز تا یک هفته جدا می‌شود. این روش درد کمی دارد و نیاز به بیهوشی ندارد.</p>
<h4>۵. سن شروع غربالگری سرطان پروستات چه زمانی است؟</h4>
<p>برای مردان عادی، از سن ۵۰ سالگی و برای کسانی که سابقه خانوادگی دارند، از ۴۵ سالگی توصیه می‌شود سالانه آزمایش PSA و معاینه انگشتی انجام دهند.</p>
<h3>سلامت دستگاه ادراری خود را جدی بگیرید</h3>
<p>برای دریافت نوبت ویزیت با متخصص اورولوژی و انجام مشاوره محرمانه، همین حالا اقدام کنید.</p>
<p>
<a>📞 ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/urology-clinic/';
...
<h2>سؤالات متداول</h2>
<strong>چقدر طول می‌کشد زخم وریدی خوب شود؟</strong>
<p>زخم‌های وریدی متوسط معمولاً در ۳ تا ۴ ماه با درمان فشاری مناسب بهبود می‌یابند. زخم‌های وسیع ممکن است ۶ تا ۱۲ ماه نیاز داشته باشند.</p>
<strong>آیا جوراب واریس را همیشه باید پوشید؟</strong>
<p>پس از بهبود زخم، استفاده‌ی دائمی از جوراب واریس درجه ۲ برای جلوگیری از عود توصیه می‌شود.</p>
<strong>آیا بدون درمان واریس، زخم بهبود می‌یابد؟</strong>
<p>ممکن است موقتاً بهبود یابد، اما عود قطعی است. درمان ریشه‌ای نارسایی وریدی برای بهبود پایدار ضروری است.</p>
<h3>برای ارزیابی تخصصی زخم وریدی</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>
<h3>سایر زخم‌ها</h3>
<p>
<a href="/service/arterial-ulcer/">زخم شریانی</a> · <a href="/service/diabetic-foot/">زخم پای دیابتی</a> · <a href="/service/chronic-wound/">زخم مزمن</a>
</p>' WHERE path = '/service/venous-ulcer/';
...
<h2>سؤالات متداول</h2>
<strong>آیا هپاتیت B واقعاً قابل درمان نیست؟</strong>
<p>هپاتیت B مزمن قابل کنترل کامل است (سرکوب ویروس) اما شفای کامل نادر است. در حال حاضر داروهای جدید در حال توسعه هستند.</p>
<strong>هزینه‌ی درمان هپاتیت C چقدر است؟</strong>
<p>به لطف تولید داخلی DAAها در ایران، هزینه‌ی درمان هپاتیت C در مقایسه با ۱۰ سال پیش به‌شدت کاهش یافته و معمولاً تحت پوشش بیمه قرار می‌گیرد.</p>
<h3>مشاوره و درمان هپاتیت (محرمانه)</h3>
<p>
<a>☎ ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/viral-hepatitis/';
...
<p>✓<br></p>
<p>ترک سیگار و قلیان (نیکوتین دشمن اصلی خون‌رسانی به زخم است)</p>
<h2>سوالات متداول بیماران زخم</h2>
<h4>۱. آیا درمان زخم دیابتی نیاز به بستری دارد؟</h4>
<p>در اکثر موارد خیر. بیش از ۹۰٪ زخم‌های دیابتی به صورت سرپایی در کلینیک درمان می‌شوند. فقط در صورت وجود عفونت استخوان (استئومیلیت) یا گانگرن گسترده، نیاز به بستری و جراحی است.</p>
<h4>۲. هزینه درمان زخم چقدر است؟</h4>
<p>هزینه بستگی به وسعت زخم و نوع پانسمان دارد. ما قبل از شروع درمان، برآورد هزینه را به شما اعلام می‌کنیم. همچنین با بسیاری از بیمه‌های تکمیلی قرارداد داریم.</p>
<h4>۳. آیا خدمات تعویض پانسمان در منزل دارید؟</h4>
<p>بله، برای بیمارانی که امکان حرکت ندارند، تیم پرستاری متخصص ما پس از ویزیت اولیه توسط پزشک، برای تعویض پانسمان به منزل اعزام می‌شوند.</p>
<h4>۴. چه زمانی باید نگران زخم شوم؟</h4>
<p>اگر زخم دارای بوی بد، ترشحات چرکی سبز یا زرد، قرمزی گسترش یابنده در اطراف و یا تب باشد، نشانه عفونت است و باید فوراً مراجعه کنید.</p>
<h3>بهبود زخم، بازگشت به زندگی</h3>
<p>برای دریافت نوبت ارزیابی زخم و مشاوره با تیم تخصصی، همین حالا اقدام کنید.</p>
<p>
<a>📞 ۰۲۱-۹۱۳۰۳۱۳۲</a>
</p>' WHERE path = '/service/wound-clinic/';
