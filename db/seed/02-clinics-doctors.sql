...
-- ------------------------------------------------------------
--  چهار عضو تازه‌ی تیم درمان
--
--  دو نفرشان پزشک نیستند: خانم دکتر مریم اسلامی و آقای مرتضی
--  مهدوی مشاور روان‌شناس‌اند. اسکیمای Physician برایشان نمی‌زنیم
--  و شماره‌ی پروانه‌شان هم نظام پزشکی نیست — is_physician = 0 و
--  license_label همین را نگه می‌دارد.
--
--  «دکتر» در نام خانم اسلامی از دکترای مدیریت سلامت می‌آید نه
--  مدرک پزشکی، و روی صفحه هم با عنوان «مشاور روان‌شناس» می‌آید تا
--  بیمار اشتباه نگیرد. آقای مهدوی اصلاً عنوان دکتر ندارد.
--
--  صفحه‌ی این چهار نفر در سایت قبلی نبود، پس آدرس تازه‌اند و
--  قرارداد ۸۲ آدرس ایندکس‌شده را نقض نمی‌کنند.
--  عکسشان هنوز نرسیده؛ تا آن موقع قالب گرادیان می‌گذارد.
-- ------------------------------------------------------------
INSERT INTO pages (path, path_norm, type, slug, title, meta_title, meta_desc, sort) VALUES
  ('/team/دکتر-نیکی-تدین/', '/team/دکتر-نیکی-تدین/', 'doctor', 'دکتر-نیکی-تدین',
   'دکتر نیکی تدین',
   'دکتر نیکی تدین | متخصص جراحی عمومی، فوق تخصص جراحی عروق',
   'دکتر نیکی تدین، متخصص جراحی عمومی و فوق تخصص جراحی عروق در همراه کلینیک تجریش تهران. نظام پزشکی ۹۹۲۲۲. رزرو نوبت و مشاوره.',
   130),
  ('/team/دکتر-مهدی-شفیعی-ثابت/', '/team/دکتر-مهدی-شفیعی-ثابت/', 'doctor', 'دکتر-مهدی-شفیعی-ثابت',
   'دکتر مهدی شفیعی ثابت',
   'دکتر مهدی شفیعی ثابت | متخصص بیماری‌های مغز و اعصاب',
   'دکتر مهدی شفیعی ثابت، متخصص بیماری‌های مغز و اعصاب (نورولوژی) در همراه کلینیک تجریش تهران. نظام پزشکی ۱۱۱۴۵۹. رزرو نوبت و مشاوره.',
   135),
  ('/team/دکتر-مریم-اسلامی/', '/team/دکتر-مریم-اسلامی/', 'doctor', 'دکتر-مریم-اسلامی',
   'دکتر مریم اسلامی',
   'دکتر مریم اسلامی | مشاور روان‌شناس',
   'دکتر مریم اسلامی، مشاور روان‌شناس با کارشناسی ارشد روان‌شناسی و دکترای مدیریت سلامت، در همراه کلینیک تجریش تهران.',
   140),
  ('/team/مرتضی-مهدوی/', '/team/مرتضی-مهدوی/', 'doctor', 'مرتضی-مهدوی',
   'مرتضی مهدوی',
   'مرتضی مهدوی | مشاور روان‌شناس',
   'مرتضی مهدوی، مشاور روان‌شناس و کارشناس ارشد روان‌شناسی در همراه کلینیک تجریش تهران. شماره عضویت سازمان نظام روان‌شناسی ۱۱۵۰۹.',
   145)
ON DUPLICATE KEY UPDATE title = VALUES(title),
  meta_title = VALUES(meta_title), meta_desc = VALUES(meta_desc), sort = VALUES(sort);

INSERT INTO doctors
  (page_id, name, specialty, fellowship, license_no, license_label, is_physician, sort) VALUES
  ((SELECT id FROM pages WHERE path = '/team/دکتر-نیکی-تدین/'),
   'دکتر نیکی تدین', 'متخصص جراحی عمومی',
   'فوق تخصص جراحی عروق', '99222', NULL, 1, 130),

  ((SELECT id FROM pages WHERE path = '/team/دکتر-مهدی-شفیعی-ثابت/'),
   'دکتر مهدی شفیعی ثابت', 'متخصص بیماری‌های مغز و اعصاب',
   NULL, '111459', NULL, 1, 135),

  ((SELECT id FROM pages WHERE path = '/team/دکتر-مریم-اسلامی/'),
   'دکتر مریم اسلامی', 'مشاور روان‌شناس',
   'کارشناس ارشد روان‌شناسی · دکترای مدیریت سلامت', NULL, NULL, 0, 140),

  ((SELECT id FROM pages WHERE path = '/team/مرتضی-مهدوی/'),
   'مرتضی مهدوی', 'مشاور روان‌شناس',
   'کارشناس ارشد روان‌شناسی', '11509', 'عضویت سازمان نظام روان‌شناسی و مشاوره', 0, 145)
ON DUPLICATE KEY UPDATE
  specialty = VALUES(specialty), fellowship = VALUES(fellowship),
  license_no = VALUES(license_no), license_label = VALUES(license_label),
  is_physician = VALUES(is_physician),
  page_id = VALUES(page_id), sort = VALUES(sort);

-- ------------------------------------------------------------
--  اتصال به بخش‌ها
--
--  دکتر شفیعی ثابت (نورولوژی) و دکتر رضایی کلانتری (اینترونشنال
--  رادیولوژی) عمداً به بخشی وصل نشده‌اند؛ هیچ‌کدام از ۹ بخش فعلی
--  دقیقاً تخصصشان نیست و حدس‌زدن بدتر از خالی گذاشتن است.
--  در فهرست پزشکان دیده می‌شوند و صفحه‌ی خودشان را دارند.
-- ------------------------------------------------------------
INSERT INTO doctor_clinic (doctor_id, clinic_id)
SELECT d.id, c.id FROM doctors d, clinics c WHERE
     (d.name = 'دکتر نیکی تدین'   AND c.slug = 'cardiology')
